...
for(i in 1:length(PT)){
  ...
  model1.1all <- MCMCglmm(chronic_group ~ sex + age + I(age^2) + pc1 + pc2 + pc3 +pc4 + pc5 +pc6+ pc7+ pc8 +pc9 +pc10 +gen_batch +array +assessment_centre + score, 
                          data = allData, family="ordinal", 
                          prior = prior1.1all, nitt=212000, thin=50, burnin=12000)             
  
  sink(paste0("/sdata/images/projects/GENSCOT/1/andrew/painMDD/results/pgrs-ukb/mcmc_pgrsPAINgrp_",PT[i],"_",today,".txt"),append=FALSE)
  
  print(paste("Chronic pain group at ",PT[i]))
  
  print(paste0("Summary of results for thtreshold ",PT[i]))
  print(summary(model1.1all))
  
  pdf(paste0("/sdata/images/projects/GENSCOT/1/andrew/painMDD/results/pgrs-ukb/plot_pain_pgrsPAINgrp_",PT[i],"_",today,".pdf"))
