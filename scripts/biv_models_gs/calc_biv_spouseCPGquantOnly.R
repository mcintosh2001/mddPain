...
sink(paste0(locations$results,"/ace-bivar-gs/spouse_rg_rpCPGquant",today,".txt"),append=FALSE)
cat("Bivariate correlations with CPGquant\n")
sink()

for(i in 1:(length(traits))){
  
 
    asr.formula <- paste("fixed=cbind(CPGquant,",traits[i],") ~trait + trait:sex + trait:age + trait:I(age^2)")
    ...
    asr.model.nulle<- asreml(fixed=as.formula(asr.formula)
                            ...
                            ,family=asreml.gaussian(link="identity",dispersion=NA)
                            ,maxiter=15000,trace=FALSE,na.method.X="include",workspace=8e+08)
    sink(paste0(locations$results,"/ace-bivar-gs/spouse_rg_rpCPGquant",today,".txt"),append=TRUE)
