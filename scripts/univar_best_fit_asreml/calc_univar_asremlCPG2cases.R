...
sink(paste0(locations$results,"/ace-univar-gs/asreml_CPG2casesbestfit",today,".txt"),append=FALSE)
        
    asr.model.sib<- asreml(fixed=CPG2cases~ sex + age + I(age^2)
                           ...
                           ,workspace=8e+08)      
#     
    asr.model.spouse<- asreml(fixed=CPG2cases~sex + age + I(age^2)
...
asr.model.spouse.young<- asreml(fixed=CPG2cases~sex + age + I(age^2)
...
asr.model.spouse.old<- asreml(fixed=CPG2cases~sex + age + I(age^2)
...
asr.model.spouse.old.young<- asreml(fixed=CPG2cases~sex + age + I(age^2)
                              ,random= ~ped(id) + id(spouse) +id(old_household)+id(young_household)
                              ,ginverse=list(id=ainv)
                              ,data=dataframe
                              ,maxiter=15000,trace=FALSE
                              ,na.method.X="omit"
                              ...
                              ,workspace=8e+08)      
#     
    asr.model.sib.spouse<- asreml(fixed=CPG2cases~sex + age + I(age^2)
...
#     
#     
    asr.model.sib.spouse.young<- asreml(fixed=CPG2cases~sex + age + I(age^2)
                                        ...
                                        ,workspace=8e+08)      
#     
    asr.model.sib.spouse.old<- asreml(fixed=CPG2cases~sex + age + I(age^2)
...
asr.model.young.old<- asreml(fixed=CPG2cases~sex + age + I(age^2)
...
#     
    ...
    asr.model.nulle<- asreml(fixed=CPG2cases ~ sex + age + I(age^2)
                            ,random= ~ped(id)
                             ,ginverse=list(id=ainv)
                             ,data=dataframe
                             ,maxiter=15000,trace=FALSE
...
diffloglikg <- 2*(asr.model.spouse.old.young$loglik-asr.model.spouse$loglik)
...
#calculating significance between young and old model and nulle model  
