...
temp<-subset(dataframe, dep_status==0)

sink(paste0(locations$results,"/pgrs-gs/pgcpgrs_into_painMDDexcluded",today,".txt"),append=FALSE)
...
for(i in 1:(length(traits))){
  ...
  sink(paste0(locations$results,"/pgrs-gs/pgcpgrs_into_painMDDexcluded",today,".txt"),append=TRUE)
