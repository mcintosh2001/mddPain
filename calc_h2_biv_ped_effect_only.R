...
formula <- paste("fixed=",univtraits[i]," ~1 + sex + age + I(age^2)")

print(formula)
asr.model<- asreml(fixed= as.formula(formula), random =~ ped(id), ginverse=list(id=ainv), data=dataframe, maxiter=500,trace=FALSE,na.method.X="omit")
...
for(i in 1:(length(traits) - 1)){
  for(j in (i+1):length(traits)){
    ...
    dataframe$test1<-dataframe[,traits[i]]
    dataframe$test2<-dataframe[,traits[j]]
    
    asr.model<- asreml(cbind(test1,test2)~trait+trait:sex+trait:age+trait:I(age^2)
    ...
    # No covariance
    asr.model.null<- asreml(cbind(test1,test2)~trait+trait:sex+trait:age+trait:I(age^2)
...
## Remove model generated traits    
	dataframe$test1 <- NULL
	dataframe$test2 <- NULL
